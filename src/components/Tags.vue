<template>
	<div id="tags">
		<div class="column">
			<div class="card" v-for="(item,index) in data.Allarticles" :key="item.id+item.type" v-show="name == item.parent || !name"
			 ref='tags'>
				<router-link :to="{name:'details',params:{'parent':item.parent,'id':item.id}}">
					...
					<article class="card-content" role="article">
						...
						<div class="content">
							<p class="pre" v-if="index == 0">{{item.describe}}</p>
			...
			data: {
				handler: function(val, oldVal) {
					if (this.$route.meta.BreadName) {
						let some = val.Allarticles.some(el => {
							return el.parent == this.name
						})
						if (!some) {
							this.$router.push({
								name: 'NotFound'
